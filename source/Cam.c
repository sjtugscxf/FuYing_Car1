...
int road_B_near=10;//用于观察较近处的路宽判断是否会有分道，road_B下标，越小越近，其值与road_width_thr锁定
                  ...
                  //目前CAR1--10  CAR2--0
int road_width_near=70;//该值通过观察check_near对应行的正常路宽来确定
int road_B_far=40;
...
void Cam_B(){
    ...
    for(int i_valid=0;i_valid<(30-5);i_valid++) {
      if ((road_B[i_valid+5].left-road_B[i_valid].left) >= -5 && (road_B[i_valid+5].left-road_B[i_valid].left) < 10 && (road_B[i_valid].right-road_B[i_valid+1]) > 15){
        road_state=4;
    ...
    int bad_flag = 0;
    if (road_state==4){
      for(int i=0;i<(ROAD_SIZE-1) && flag_valid_row==0;i++){
        if (last_row==-1){
        if (abs(road_B[i].width-road_B[i+1])>5){
          last_row=i;
          if (last_row > 0){
            obstacle_diff = (road_B[last_row]-road_B[0])/last_row;
          }
          obstacle_mid = road_B[i].mid;
        }
        }
        else{
      ...
      case 3:   //环岛
        ...
        case 2://入环岛
          ...
          for(int i=1;i<ROAD_SIZE;i++){   //利用roundabout_choice给mid加偏移量
            if(roundabout_choice==1) road_B[i].mid = constrain(0,CAM_WID/2-50,road_B[i].mid-50);
            else if(roundabout_choice==2) road_B[i].mid = constrain(CAM_WID/2+50,CAM_WID,road_B[i].mid+50);
          }
          if(road_B[0].width<100 && time_cnt>100){ //如果路宽恢复正常，认为完成入岛//我猜不如下面的时间控制方式有效
            roundabout_state=3;
            time_cnt=0;
          }
        /*
        ...
        case 3://在环岛内，看不到出岛，当做弯道行驶
          ...
          if(car_type==leader){
            ...
            if(roundabout_choice==1)
              if(road_B[49].mid<CAM_WID/2 && time_cnt>=500){
                roundabout_state=4;
                time_cnt=0;
              }
            ...
            if(road_B[10].width>55 && road_B[10].width<65
