...
void Cam_B(){
    ...
    //区分环岛与十字的延长线法————————————————————
    if(state_set==0 ){     //若没有检测到环岛，且不在十字、终点状态下，则进行拐点（jump）检测，如下：//太精细的计算不适合，所以改成一个简单的
      int cnt=0;
    ...
    if(state_set==0){
      ...
      for(int i_valid=0;i_valid<(50-3) && flag_cross==0;i_valid++)     //寻找十字弯
      {
        ...
        if (flag_wide==1){
          if ((road_B[i_valid].right-road_B[i_valid].left) < 30){
            break;
          }
        }
     ...
     switch(road_state)
    {
      ...
      case 3:   //环岛
        ...
        case 1://检测到环岛
          ...
          if(isWider(road_B_near,110)){//如果路过于宽，认为出现分叉，开始转弯     新车摄像头可能不一样
            roundabout_state=2;
            time_cnt=0;
          }
        case 2://入环岛
